...
Cango = (function(CangoCore)
{
  ...
  CangoCore.prototype.drawHTMLText = function(str, options)
  {
    const opts = (typeof options === 'object')? options: {},   // avoid undeclared object errors
          ...
          " translate(-100%,0%); transform-origin:100% 0% ",
          " translate(0%,-50%); transform-origin:0% 50%  ",
          " translate(-50%,-50%); transform-origin:50% 50%  ",
          " translate(-100%,-50%); transform-origin:100% 50%  ",
          " translate(0%,-100%); transform-origin:0% 100%  ",
          " translate(-50%,-100%); transform-origin:50% 100%  ",
          " translate(-100%,-100%); transform-origin:100% 100%  "];
    let lorg = 1,
        styleTxt = "";
    ...
    styleTxt = lorgWC[lorg]+"; color:"+txtCol+"; font-family:"+family+"; font-size:"+size+"px; font-weight:"+weight;
    // handle transforms in insertion order NOTE: transforms are applied from the right
    for (let prop in opts)  // from ES2015 loop is in insertion order 
    {
      if (prop === "degs")
      {
        styleTxt = " rotate(-"+opts[prop]+"deg) "+styleTxt;
      }
      if (prop === "x")
      {
        const p = this.toPixelCoords(opts[prop], 0);
        styleTxt = " translateX("+p.x+"px) "+styleTxt;
      }
      if (prop === "y")
      {
        const p = this.toPixelCoords(0, opts[prop]);
        styleTxt = " translateY("+p.y+"px) "+styleTxt;
      }      
    }
    styleTxt = " transform: "+styleTxt;
...
/* &thetasym; */ '\u03D1': {width:21, cdata:['M',1,10,'L',2,12,4,14,6,14,7,13,7,11,6,6,6,3,7,1,8,0,10,0,12,1,14,4,15,6,16,9,17,14,17,17,16,20,14,21,12,21,11,19,11,17,12,14,14,11,16,9,19,7]}
  ...
  CangoCore.prototype.drawVectorText = function(str, options)
  {
    ...
    const pathData = hersheyFont.stringToCgo2D(str, size/this.xscl, lorigin);   // convert px size to WC
    ...
    // handle transforms in insertion order NOTE: transforms are applied from the right
    for (let prop in opts)  // from ES2015 loop in insertion order 
    {
      if ( prop === "degs" || prop === "scl" || prop === "x" || prop === "y")
      {
        txtObj.setProperty(prop, opts[prop]);
        if (bbox !== undefined)
        {
          bbox.setProperty(prop, opts[prop]);
        }
      } 
    }
    this.render(txtGrp);
